...
飞书文档优先使用 XML 写入并显式控制表格列宽：

- Top 3 总表优先收敛为 6 列：序号、国家/主题、承接店、类型、关键数据、判断。把候选核心 GMV、ROAS、店外占比和来源店铺数合并在“关键数据”单元格；“判断”列应是最宽列。不要把这些指标拆成 8 个以上窄列，造成逐字换行和异常大空白。
- 使用 `<colgroup><col width="..."/></colgroup>` 指定列宽；页面宽度变化时按比例缩放，不固定行高。Markdown 或其他宿主无法控制列宽时，也沿用 6 列信息分组。
- 每个补品按“中文短名/原始标题 → 商品图 → 结构化证据”排列。证据至少分为“商品定位、大盘表现、投入与渠道、补品判断、数据来源”五组；可用两列字段表或同方案对比表，但禁止把全部指标继续堆成一条竖线分隔的长段落。
- 图片保留为独立块并紧邻自己的证据表；不要为了横向对比把大图塞进多列窄表。
...
- Top 3 在飞书中没有 8 列以上拥挤布局；判断列足够宽，未因逐字换行撑出异常大空白。
...
- 每个补品的上述信息按“商品定位、大盘表现、投入与渠道、补品判断、数据来源”结构化呈现，不是单段指标堆叠。
