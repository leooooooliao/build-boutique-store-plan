...
从 `v1.3.0` 起，Skill 每次启用时都会先自动检查官方 GitHub Release；只有远端版本更高才下载完整安装包并更新。检查或更新失败不会阻断本次业务分析。

`v1.4.0` 起，客户货盘和营销参谋统一使用一个报告周期；每个精品店方案内同时呈现现有好品和对应补品，文末再按三个方案分别提供简短的 AI 延伸建议。补品证据会展示总 GMV、平均客单价、广告消耗和 `TR（估）`。

`v1.4.1` 起，Top 3 总览收敛为适合飞书页面宽度的 6 列结构；营销参谋补品改为“商品图 + 结构化证据表”，避免窄列、异常空白和整段指标堆叠。

安装 `v1.3.0` 或更高版本即表示启用这一自动更新机制；若当前 AI/宿主要求联网或写入审批，只会申请一次，拒绝后仍使用当前版本继续。
...
4. 如果已有旧版，请备份后用完整新目录覆盖；确认 VERSION、SKILL.md、references/interaction.md、references/data-contract.md 可访问。
5. 安装完成后返回：是否成功、版本号、加载位置或方式，以及一段可直接复制的第一次使用提示词。
6. 从 v1.3.0 起，每次新任务触发本 Skill 时先按 SKILL.md 检查官方 GitHub Release；有新版就更新完整目录并重新读取新版 SKILL.md，同一任务只检查一次。
7. 无法联网、下载、解压或持久安装时，请说明限制并让我上传 ZIP，不得假装安装成功或更新成功；版本检查失败时继续使用当前版。
8. 完整报告必须实际查询营销参谋，不能因找不到另一个 Skill、浏览器未授权或页面操作困难而静默跳过；应先提示我开启本地浏览器能力。
9. 只有我确认安装成功且你具备 GitHub 写入能力时，才询问是否自愿登记一次安装；不要重复登记。
```

## 自动更新怎么生效

- **已经安装 `v1.3.0` 或更高版本：** 每次新任务启用时自动检查一次；同版不下载，有新版才更新。
- **仍是 `v1.2.0` 或更早版本：** 旧版没有更新器，无法凭空发现新版。请先用上面的安装提示词手动覆盖安装一次最新版；以后即可自动检查。也就是说，v1.2.0 或更早版本需要这一次人工引导升级。
- **更新成功：** AI 重新读取新版 `SKILL.md` 后继续，不需要用户重新提供业务数据。
- **网络或写入权限失败：** 使用当前版本继续，只说明一次真实状态，不反复询问。

自动检查不会增加 Release ZIP 下载数；只有实际发现新版并下载 ZIP 时才会产生下载事件。自动更新也不会重复提交安装登记。
...
先不要分析，请按 Skill 的首次交互提示我开启本地浏览器，并直接给我两条完整拉数网址，告诉我如何提供两份完整表格和一个准确报告周期。客户货盘和营销参谋使用同一周期；完整报告必须包含营销参谋补品验证。
```

AI 的第一条回复应先完成浏览器预检，再直接提供下面两条完整网址，不能只写表名或编号。两个报表都要把“商家”改成同一个目标商家：

1. ID 层级数据：<https://mmm.tiktok-row.net/apps/analytics/biportal/report/edit/1324598>（经营指标的唯一事实源）。
2. 名称对照数据：<https://mmm.tiktok-row.net/apps/analytics/biportal/report/edit/1324606>（只用于理解商品名和具体店铺名）。
