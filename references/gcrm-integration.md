...
3. 优先读取 GMV Top 50；有效候选不足时扩到 Top 100，再在确有有效数据时看飙升榜。宽泛一级类目干扰过大时，AI 必须自动展开并选择最贴近主题的二级类目，例如 `美妆个护 → 美容、个护电器`；不得把展开、滚动和点击回推给用户。
...
- 每个补品保留：
  ...
  - 平均客单价
  - 广告消耗区间
  - `TR（估）`；唯一公式为 `广告消耗区间中点 ÷ 总 GMV 区间中点`
...
- 页面区间保持原样。TR 是推导值，必须标注“估”；区间无法形成可靠中点时写“不可计算”并保留原因。TR 只辅助判断入局所需广告投入，不单独决定补品。

## 图片与链接

- 优先保存原始图片 URL，并在飞书文档中同时放图片和可点击源链接。
- 没有稳定图片 URL 时，可截取带商品卡片及当前筛选条件的页面截图。
- 图片下方注明 `国家｜类目｜报告周期｜product_id`，避免离开上下文后误用。
